...
Na execução validada do projeto, com o dataset real baixado em `data/raw/owid-energy-data.csv`, foram realizadas 7 execuções por consulta. A consulta sobre a fato original retornou 261 linhas, com melhor tempo de 6,631 ms e média de 8,078 ms. A consulta sobre a tabela agregada retornou as mesmas 261 linhas, com melhor tempo de 2,254 ms e média de 3,064 ms.

O ganho médio observado foi de **2,64x** ao usar `dw.fact_energy_generation_annual_grouped`. Os valores também estão registrados em `outputs/performance_summary.md`.
